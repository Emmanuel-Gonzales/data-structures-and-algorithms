...
const longestString = (arr) => {
  // Solution code here...
  let coppyArr = [...arr];
  coppyArr.sort((a, b) => b.length - a.length);
  return arr.indexOf(coppyArr[0]);
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(phoneNum =>phoneNum.replace(/[{()}]/g, '').replace(/[-]/g, '').replace(/\s/g, ''));
};
...
const allHappy = (arr) => {
  // Solution code here...
  let boolArr = arr.map(arr => arr.includes(':)'));
