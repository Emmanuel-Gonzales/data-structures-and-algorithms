...
const screenForNames = (arr) => {
  // Solution code here...
  return arr.filter(person => /^(Mr\.||Mrs\.||Ms\.||Dr\.)\s[a-zA-Z]+$/gm.test(person));
};
...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(indx => {
    let partWord = indx.slice(1)
    return indx.charAt(0).toUpperCase() + partWord;
  });
};
...
const detectTicTacToeWin = (board) => {
  ...
  if(helpCheck(0,0,0,1,0,2)){
    return true;
  }else if(helpCheck(1,0,1,1,1,2)){
    return true;
  }else if(helpCheck(2,0,2,1,2,2)){
    return true;
  }else if(helpCheck(0,0,1,0,2,0)){
    return true;
  }else if(helpCheck(0,1,1,1,2,1)){
    return true;
  }else if(helpCheck(0,2,1,2,2,2)){
    return true;
